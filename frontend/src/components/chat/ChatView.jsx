...
import { resolveWs, chatApi } from "./api";
...
export default function ChatView({ user, llmSessionId }) {
  ...
  const loadSessions = useCallback(async () => {
    try {
      const data = await chatApi.listSessions(user);
      setSessions(data);
  ...
  const loadMessages = useCallback(
    async (chatSessionId) => {
      ...
      try {
        const data = await chatApi.listMessages({ userId: user, chatSessionId });
        setMessages(
          data.map((msg) => ({
            id: msg.id,
            role: msg.role,
  ...
  useEffect(() => {
    ...
    const wsUrl = resolveWs(
      `/chat?chat_session_id=${selectedSessionId}&llm_session_id=${llmSessionId}&user_id=${user}`
    );
  ...
  }, [selectedSessionId, llmSessionId, user, setSessions]);
  ...
  }, [user, llmSessionId, loadSessions]);
        ...
        <div className="border-b-2 border-black px-4 py-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs uppercase tracking-wide text-gray-600">User: {user}</p>
            </div>
            <button
              type="button"
